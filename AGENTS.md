...
- **Тесты:** `src/test/kotlin/me/vektory79/vulkan/kotlin/` — интеграционные тесты обёртки
...
| `./gradlew test --tests "ClassName"` | Запуск конкретного тест-класса |
...
Библиотека-обёртка тестируется **интеграционными тестами**, которые проверяют корректность обёрток через реальные вызовы Vulkan API. Тестирование отдельных геттеров, сеттеров и конструкторов не производится — это тестирование LWJGL, а не нашей обёртки.
